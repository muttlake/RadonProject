function varargout = CTSimulator(varargin)
% CTSIMULATOR MATLAB code for CTSimulator.fig
%      CTSIMULATOR, by itself, creates a new CTSIMULATOR or raises the existing
%      singleton*.
%
%      H = CTSIMULATOR returns the handle to a new CTSIMULATOR or the handle to
%      the existing singleton*.
%
%      CTSIMULATOR('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in CTSIMULATOR.M with the given input arguments.
%
%      CTSIMULATOR('Property','Value',...) creates a new CTSIMULATOR or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before CTSimulator_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to CTSimulator_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help CTSimulator

% Last Modified by GUIDE v2.5 05-Nov-2017 23:33:20

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @CTSimulator_OpeningFcn, ...
                   'gui_OutputFcn',  @CTSimulator_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before CTSimulator is made visible.
function CTSimulator_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to CTSimulator (see VARARGIN)

% Choose default command line output for CTSimulator
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes CTSimulator wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = CTSimulator_OutputFcn(hObject, eventdata, handles) 
